  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
                ...
                void main() {
                    // Gradiente vertical - más oscuro abajo
                    float gradient = smoothstep(-1.5, 1.0, vPosition.y);
                    float brightness = mix(0.3, 1.0, gradient);
    ...
    // Luces adicionales para iluminación uniforme
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.6);
    scene.add(ambientLight);

    // Luz desde la izquierda para balancear
    const leftLight = new THREE.PointLight(0xffffff, 0.8, 100);
    leftLight.position.set(-5, 0, 3);
    scene.add(leftLight);

    // Luz desde la derecha
    const rightLight = new THREE.PointLight(0xffffff, 0.8, 100);
    ...
    >
      ...
      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-20 md:pb-32 text-center">
        <div className="max-w-3xl px-4 animate-fade-in-long">
          <h1 className="text-sm font-mono font-poppins tracking-widest text-sky-400 uppercase">
            {title}
          </h1>
          <p className="mt-4 text-3xl md:text-5xl font-black font-poppins leading-tight">
            {subtitle}
          </p>
          <p className="mt-6 max-w-xl mx-auto text-base font-poppins leading-relaxed text-gray-400">
            {description}
          </p>
        </div>
      </div>
